...
p1 <-ggplot(combined_ds, 
  ...
  annotate("text", x=2800, y=0.7, label=paste0("Singapore's Median:",median(combined_ds$`Unit Price ($ PSF)`)), 
           size=3, color="red") +
...
p2 <- ggplot(df_Aavg, aes(x = `Planning Region`, y = p_z, label = p_z)) +
  geom_col(aes(fill = factor(p_ztype)), position = position_dodge2(width = 2), width = 0.5) +
  scale_fill_colorblind(name = "Price Deviation", labels = c("Above Singapore's Median", "Below Singapore's Median")) +
