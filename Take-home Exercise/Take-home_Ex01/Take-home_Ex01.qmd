...
### 3.1.2  Questions for the visualisation to answer

For the visualisation below, we will attempt to answer the following questions:

1.    What is the pricing for these 3 sub-markets?
...
**Apartment Tenure(Lease vs Freehold)**
```{r}
#| echo: False
df.c %>%
  filter(`Type of Area`== 'Strata',`Property Type` == 'Apartment', `Quarter` == '24Q1', `Tenure`!= 'Freehold') %>%
  ...
  scale_x_continuous(labels = scales::comma, 
                     ...
                     limits = c(0,8000000)) +
  theme_economist() + 
  ggtitle("24Q1 (Lease): Distribution of Transacted Price by Planning Region") +
...
#| eval: False
df.c %>%
  filter(`Type of Area`== 'Strata',`Property Type` %in% c('Apartment','Condominium','Executive Condominium'), `Quarter` == '24Q1', `Tenure`!= 'Freehold') %>%
  ggplot(aes(x = `Transacted Price ($)`,
           y = `Planning Region`,
           fill = factor(stat(quantile))
           )) +
