...
```{r}
ggplot(data = land_df ,
       aes(x = `Transacted Price per Unit ($)`,
           y = Quarter,
           fill = factor(stat(quantile))
           )) +
  stat_density_ridges(
...
```{r}
ggplot(data = land_df, 
       aes(y = `Transacted Price per Unit ($)`,            x = `Property Type`)) +
  geom_boxplot()+
  facet_grid(~Quarter) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
...
mean_strata_by_property_type <- strata_df %>%
  ...
  summarise(mean_price = mean(`Transacted Price per Unit ($)`))

ggplot(strata_df,
       aes(x=`Area (SQM)`, 
           y = `Transacted Price per Unit ($)`,
           color = `Property Type`)) +
...
quarter_means <- df2 %>%
  group_by(Quarter) %>%
  summarise(mean_tprice = mean(`Transacted Price per Unit ($)`))

ggplot(data = df2,
       aes(y=`Transacted Price per Unit ($)`,
           x=`Quarter`)
       )+
  geom_violin()+
...
```{r}
ggplot(df,
       aes(y = df2$`Transacted Price per Unit ($)`)) +
  geom_boxplot()
```
